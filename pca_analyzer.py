...
def clean_numeric_data(data):
    ...
    for col in data_clean.columns:
        ...
        if data_clean[col].dtype == 'object':
            ...
            try:
                ...
                if data_clean[col].isnull().mean() > 0.5:  # More than 50% NaN
                    problematic_columns.append(col)
                    print(f"   ⚠️  Column '{col}' has too many non-numeric values - will be dropped")
                else:
    ...
    # Final check for any remaining non-numeric values
    non_numeric_cols = data_clean.select_dtypes(include=['object']).columns
    if len(non_numeric_cols) > 0:
        print(f"   ⚠️  Still found non-numeric columns: {list(non_numeric_cols)} - dropping them")
